...
const buildPath = (shape, curviness, ribbonWidth, VIEW_W, CY) => {
...
const TextLoop = ({
  ...
  const d = useMemo(
    () => path || buildPath(shape, curviness, ribbonWidth, VIEW_W, CY),
    [path, shape, curviness, ribbonWidth, VIEW_W, CY],
  );
  ...
  useEffect(() => {
    ...
    const safeSpeed = Math.max(speed, 10);
    const state = { offset: 0 };

    const tween = gsap.to(state, {
      offset: direction === "reverse" ? -length : length,
      duration: length / safeSpeed,
      ease: "none",
      repeat: -1,
      lazy: false,
      force3D: true,
  ...
  return (
    <div
      ref={rootRef}
      className={`relative w-full overflow-hidden ${pauseOnHover ? "pointer-events-auto" : "pointer-events-none"} select-none ${className}`.trim()}
      onMouseEnter={() => {
    ...
    >
      ...
      >
        <path
          ...
          d={d}
          fill="none"
          stroke={ribbon && ribbonWidth > 0 ? ribbonColor : "none"}
